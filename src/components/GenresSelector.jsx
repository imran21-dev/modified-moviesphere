    ...
    return (
        <div>
          ...
          <div onClick={handleGenreSwitch} className={genreSwitch ? 'font-thin rounded-full bg-secondary border border-secondary flex items-center gap-1 justify-center px-2 py-2 cursor-pointer':'font-thin rounded-full border border-secondary flex items-center hover:bg-neutral/10 gap-1 justify-center px-2 py-2 cursor-pointer'}>Select genre {genreSwitch? <IoChevronUpSharp />:<IoChevronDownSharp />}</div>
            ...
            <ul className={genreSwitch ? 'text-sm bg-secondary/50 backdrop-blur-md py-2 rounded-2xl relative z-50 duration-300 text-left' : 'text-sm text-left hidden'}>
             {
             genres.map(genre => <li  onClick={()=>addGenre(genre)} className="hover:bg-secondary rounded-full cursor-pointer py-1 px-2" key={genre}>{genre}</li>)
             }
            
            </ul>
          </section>
          
          <section className= 'border flex justify-start rounded-3xl flex-wrap gap-2 border-secondary h-max py-3 px-2'>
           {
