...
const ReleaseUp = () => {
    const {setReleaseYearUp,releaseToolUp,setReleaseToolUp,releaseUp, setReleaseUp,} = useContext(ThemeContext)
        ...
        <div className="w-2/4">
          ...
          <label className="label">
            <span className="label-text md:text-sm text-xs">Release Year</span>
             ...
             <div className={`tooltip w-full  tooltip-bottom ${releaseToolUp ? 'tooltip-open': ''}`} data-tip="Add release date">
          <div onClick={handleGenreSwitch} className='font-thin rounded-full hover:bg-neutral/10 border  border-secondary  px-5 py-2 cursor-pointer'>{releaseUp? <h1>{releaseUp}</h1> : <h1 className="flex items-center gap-1 justify-center">Release year <IoChevronDownSharp /></h1>}</div>
          </div>
            <ul className={genreSwitch ? 'text-sm absolute rounded-xl h-52 w-full z-50 bg-secondary/50 backdrop-blur-md overflow-y-auto text-left' : 'text-sm text-left hidden'}>
             {
