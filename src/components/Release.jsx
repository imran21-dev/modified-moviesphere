    ...
    return (
        <div className="w-2/4">
          ...
          <div onClick={handleGenreSwitch} className='font-thin rounded-full hover:bg-neutral/10 border  border-secondary  px-5 py-2 cursor-pointer'>{release? <h1>{release}</h1> : <h1 className="flex items-center gap-1 justify-center">Release year <IoChevronDownSharp /></h1>}</div>
            ...
            <ul className={genreSwitch ? 'text-sm rounded-xl h-52 relative z-50 bg-secondary/50 backdrop-blur-md overflow-y-auto text-left' : 'text-sm text-left hidden'}>
             {
             years.map(year => <li onClick={()=>addYear(year)} className="hover:bg-secondary cursor-pointer rounded-full py-1 px-2" key={year}>{year}</li>)
             }
